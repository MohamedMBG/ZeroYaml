...
func TestGetInfoReturnsProcessIdentityAndCapabilities(t *testing.T) {
	...
	response, err := New(identity, &recordingJobSubmitter{}, discardLogger()).GetInfo(context.Background(), &runnerv1.GetInfoRequest{})
	if err != nil {
...
func TestPingEchoesMessageAndReportsRunnerVersion(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			response, err := New(identity, &recordingJobSubmitter{}, discardLogger()).Ping(
				context.Background(),
...
func TestPingToleratesNilRequest(t *testing.T) {
	...
	response, err := New(identity, &recordingJobSubmitter{}, discardLogger()).Ping(context.Background(), nil)
	if err != nil {
		t.Fatalf("Ping() returned an error: %v", err)
	}
