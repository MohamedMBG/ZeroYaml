...
the Control Plane can distinguish each process instance. The identity and
...
```text
level=INFO msg="run job acknowledged" job_id=0f4b1a1e-... protocol_version=runner.v1 runner_id=local-runner instance_id=... acceptance=JOB_REJECTED rejection_reason=JOB_REJECTION_RUNNER_UNAVAILABLE
```
...
revision, or command arguments, and caller-supplied identifiers are truncated
to 128 bytes.
...
protoc -I proto `
  ...
  --go-grpc_out=runner/gen `
  --go-grpc_opt=paths=source_relative `
  proto/runner/v1/runner.proto
```

The Java bindings are regenerated by the Maven protobuf plugin during
`control-plane\mvnw.cmd verify`.
